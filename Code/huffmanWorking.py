...
def encode(file_name):  # compress the file
    file = open("InputFiles/" + file_name, "rb")  # opens the file from the InputFiles directory
    ...
    for val in codebook_list:
        ...
        previous_val = val[0]
        codebook_output += "1"  # adds a zero if length is avaliable, length presented next
